...
module.exports = function(options = {})
{
    ...
    return async (ctx, next) =>
    {
        ...
        if (client_header === "no-cache" ||
            client_header === "no-store" ||
            client_header === "must-revalidate")
        ...
        if (ctx.method === "GET" || ctx.method === "HEAD")
        {
            ...
            if (data)
            {
                ...
                if (expire)
                {
                    const time_diff = new Date().getTime() - data.expire
                    const expires_in = Math.floor(expire - time_diff / 1000)
